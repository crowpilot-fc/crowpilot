...
#   2. The SBUS receiver needs a PIO program. sbus_rx.pio is assembled
#      to sbus_rx.pio.h here (the result is also committed, so an
#      Arduino IDE build without this script still works).
#
...
# Assemble the SBUS PIO program if pioasm from the rp2040 core is present.
...
if [ -n "${PIOASM}" ]; then
  ...
  echo "note: pioasm not found, using the committed sbus_rx.pio.h" >&2
fi
