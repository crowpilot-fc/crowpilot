...
| Option | Default | Meaning |
|---|---|---|
| `AIRFRAME` | `AIRFRAME_PLANE_TWIN_CARGO` | Selects the mixer. The plane airframes (`AIRFRAME_PLANE_TWIN_CARGO`, the Caribou first-flight target, and `AIRFRAME_PLANE_SINGLE`) require `ENABLE_PLANE_STAB`. `AIRFRAME_TAILSITTER_BICOPTER` is the carried alternative. `AIRFRAME_QUAD_X` is a four-motor X quad with self-leveling angle and acro rate modes. `AIRFRAME_PLANE_FLYING_WING` is a one-motor delta with two elevons and no rudder. `AIRFRAME_PLANE_VTAIL` is a one-motor plane with two ailerons and two ruddervators (the V tail mixes pitch and yaw). Both need `ENABLE_PLANE_STAB`. The remaining frames (`AIRFRAME_HEX_X`, `AIRFRAME_TRICOPTER`, `AIRFRAME_TAILSITTER_QUAD`) are reserved and halt the build. |
...
| `ENABLE_LAUNCH_ASSIST` | `0` | Hand-launch assist for the angle and horizon modes. Arm with the throttle at idle and throw the plane; a forward-accel spike starts a wings-level climb-out (launch throttle plus a nose-up hold) for a fixed window. Ends on timeout, a stick input, or disarm. |
| `LAUNCH_DETECT_ACCEL_G`, `LAUNCH_IDLE_MAX`, `LAUNCH_THROTTLE`, `LAUNCH_PITCH_DEG`, `LAUNCH_DURATION_MS`, `LAUNCH_STICK_EXIT` | `2.0`, `0.15`, `0.7`, `15`, `1500`, `0.2` | Throw-detect accel, idle throttle to arm the detector, climb-out throttle, climb-out pitch, climb-out window in ms, and the stick deflection that hands control back. |
