...
- Twin-engine cargo plane mixer (Caribou), single-engine plane mixer, flying-wing elevon mixer, V-tail mixer, the tailsitter bicopter mixer with its transition fader and flight-mode enum, and the Quad X mixer with self-leveling angle and acro rate modes.
