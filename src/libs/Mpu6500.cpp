...
// Register addresses per MPU-6500 Register Map rev 2.1.
...
// WHO_AM_I expected value per datasheet section 11.3.
constexpr uint8_t kWhoAmIExpected   = 0x70;

// PWR_MGMT_1 fields.
constexpr uint8_t kPwrResetBit      = 0x80;        // device reset
constexpr uint8_t kPwrClkPllXGyro   = 0x01;        // CLKSEL = 1, recommended

// CONFIG (0x1A) DLPF_CFG = 1 -> gyro bandwidth 184 Hz, 1.9 ms delay, internal
// sample rate 1 kHz. Matches the 1 kHz LOOP_HZ default.
constexpr uint8_t kConfigDlpf184Hz  = 0x01;

// ACCEL_CONFIG2 (0x1D) A_DLPF_CFG = 1 -> accel bandwidth 184 Hz, 5.8 ms delay,
// internal sample rate 1 kHz.
constexpr uint8_t kAccelConfig2_184Hz = 0x01;

// SMPLRT_DIV = 0 -> internal sample rate = base rate / (1 + 0) = 1 kHz.
...
bool init(uint8_t i2c_addr, uint8_t gyro_fs_sel, uint8_t accel_fs_sel) {
  ...
  if (!writeReg(kRegConfig, kConfigDlpf184Hz)) {
    return false;
  }
  ...
  if (!writeReg(kRegAccelConfig2, kAccelConfig2_184Hz)) {
    return false;
  }
  return true;
}
...
float tempRawToCelsius(int16_t raw) {
  return static_cast<float>(raw) * (1.0f / 333.87f) + 21.0f;
}
